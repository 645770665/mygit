<template>
  <div class="box" @click='selet=false'>
    <zujian1 state='123'></zujian1>
    <div>-------------------------------------</div>
    <!-- <form>
      <input type="text" name="a" value="11">
      <input type="text" name="b" value="2222">
      <button type="button" @click="tijiao">提交</button>
    </form> -->
    <input type="text" v-if="!selet" v-focus:a.b="123">
    ...
    <div class="less">
      ...
      <router-link :to="{ name: 'niubi', params: { id: ids }}">这是跳转#</router-link>
      <router-link to="/rrrrr/13">这是跳转55555</router-link>
      <button @click="rrrrr">rrrrrrrr</button>
    </div>
    <br>
    <keep-alive>
  ...
  export default {
    ...
    methods: {
      ...
      tijiao(){
        ...
        let li2 = this.$axios.post('http://localhost:3000/eee',b,config)
      }
    },
    computed: {
    ...
    mounted() {
      // ------------------------------------------------------
      // var a  =  this.$axios.get('http://localhost:3000/demo')
      var a  =  this.$axios({
        url:'/demo',
        //`baseURL`将被添加到`url`前面，除非`url`是绝对的(http://localhost:3000/demo)则不生效。
        baseURL: 'http://localhost:3000',
        method:'get',
        headers: {
          // headers设置请求头
          // Content-Type设置请求接口类型
          // 'Content-Type':'multipart/form-data',
          'Content-Type':'application/x-www-form-urlencoded',
          // Authorization设置令牌
          'Authorization':'token-nimasilecaonimabizhenniu1111111111111111111' //设置token请求头
        },
        data:{
          // 设置请求时传给后端的数据
          firstName: 'Fred'
        ...
        // params: {
        //   // 设置默认数据（params都是拼接在地址上的）
        //   ID: 12345,
        //   nima:'nimasile'
        // },
        // 分割线---------------------以下配置了解即可，开发时不会用到
        // auth: {
        //   username: 'janedoe',
        //   password: 's00pers3cret'
        // },
        // // responseType: 'json', // default，不用设置，axios响应的数据都会自动转换从json转成普通数据格式+

        // // `maxContentLength`定义允许的http响应内容的最大大小
        // maxContentLength: 2000, //定义了接收到的response响应数据的最大长度，了解有印象即可，正常不用设置

        // // `validateStatus`定义是否解析或拒绝给定的promise
        // // HTTP响应状态码。如果`validateStatus`返回`true`（或被设置为`null` promise将被解析;否则，promise将被拒绝。
        // // 用人话说就是，状态码>=200并且<300的时候执行resolve方法运行.then()，反之执行reject方法运行.catch()
        // validateStatus: function (status) {
        //   return status >= 200 && status < 300; // default，完全不需要动这一项！了解即可
        // },
